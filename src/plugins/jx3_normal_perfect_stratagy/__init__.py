...
#====================Matcher====================
normal_perfect_stratagy_query = user_matcher_group.on_regex(pattern=REGEX.奇遇攻略.value)
...
url = 'https://node.jx3box.com/serendipities?type=normal&page=1&per=14'
...
url = 'https://node.jx3box.com/serendipities?type=normal&page=2&per=14'
response = requests.get(url)
dwIDs = re.findall(r'"dwID":(\d+)', response.text)
szNames = re.findall(r'"szName":"(.*?)"', response.text)
list_str2 = szNames
...
url = 'https://node.jx3box.com/serendipities?type=perfect&page=1&per=20'
response = requests.get(url)
dwIDs = re.findall(r'"dwID":(\d+)', response.text)
szNames = re.findall(r'"szName":"(.*?)"', response.text)
list_str1 = szNames
...
# ==============handler===============
@normal_perfect_stratagy_query.handle(parameterless=[cold_down(name="奇遇攻略查询", cd_time=0)])
async def _(event: GroupMessageEvent, name: str = get_value()) -> NoReturn:
    ...
    await normal_perfect_stratagy_query.finish(msg)